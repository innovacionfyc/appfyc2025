...
// --- WATCHER ---
watch(
  () => props.categoriasIngredientes,
  (newCategoriaIngrediente) => {
    if (newCategoriaIngrediente) {
      form.nombre = newCategoriaIngrediente.nombre ?? "";
      form.descripcion = newCategoriaIngrediente.descripcion ?? "";
      form.icono = newCategoriaIngrediente.icono ?? "";
      form.color_hex = newCategoriaIngrediente.color_hex ?? "";
      form.clearErrors();
    }
  },
...
// --- ACCIONES ---
function submit() {
  if (!props.categoriasIngredientes) return;

  form.post(route("repos.categoriasIngrediente.update", props.categoriasIngredientes.id), {
    preserveScroll: true,

...
const layoutConfig = computed(() => {
  ...
  let wrapperClass = 'flex flex-col lg:flex-row gap-6 lg:gap-8 w-full h-full lg:h-[85dvh]';
  ...
  if (estiloP === 'invertido') {
    wrapperClass = 'flex flex-col lg:flex-row-reverse gap-6 lg:gap-8 w-full h-full lg:h-[85dvh]';
  } else if (estiloP === 'minimalista') {
    wrapperClass = 'flex flex-col gap-8 w-full h-auto overflow-y-auto custom-scroll pr-2 pb-16';
    temarioContainer = 'w-full flex flex-col';
    sidebarContainer = 'w-full grid grid-cols-1 lg:grid-cols-2 gap-8 items-stretch';
  }
  ...
  let moduloUlClass = 'grid gap-3 bg-slate-50/50 rounded-2xl p-5 border border-slate-100 grid-cols-1 md:grid-cols-2 flex-1 mt-auto';

  if (estiloT === 'cuadricula') {
    temarioGridClass = 'grid grid-cols-1 md:grid-cols-2 gap-6 items-stretch';
    moduloUlClass = 'grid gap-2 bg-slate-50/50 rounded-xl p-4 border border-slate-100 grid-cols-1 flex-1 mt-auto';
  } else if (estiloT === 'tarjetas') {
    temarioGridClass = 'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 items-stretch';
    moduloUlClass = 'flex flex-col gap-3 p-5 bg-white/50 border-t border-slate-100 mt-auto flex-1';
  }

  let expertosGridClass = 'space-y-3 pb-4 flex-1';
  let expertoCardClass = 'group relative bg-slate-50/50 hover:bg-white border border-transparent hover:border-slate-200 p-3 md:p-4 rounded-[1.2rem] md:rounded-[1.5rem] transition-all flex items-center gap-4 overflow-hidden shadow-sm w-full';

  if (estiloE === 'tarjetas') {
    expertoCardClass = 'group relative bg-slate-50/50 hover:bg-white border border-transparent hover:border-slate-200 p-5 rounded-2xl transition-all flex flex-col items-center text-center shadow-sm w-full';
    
    if (totalExpertos === 1) {
      expertosGridClass = 'flex flex-col items-center justify-center pb-4 flex-1 h-full min-h-[200px]';
      expertoCardClass += ' max-w-sm mx-auto shadow-md ring-1 ring-slate-100'; 
    } else if (totalExpertos === 2) {
      expertosGridClass = 'grid grid-cols-1 sm:grid-cols-2 gap-4 pb-4 flex-1 content-center min-h-[200px]';
    } else {
      expertosGridClass = 'grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-2 xl:grid-cols-3 gap-3 pb-4 flex-1';
    }
  } else {
    if (totalExpertos === 1) {
  ...
  let cardPrecioClass = 'relative overflow-hidden bg-gradient-to-br from-[#0B192C] to-[#081121] rounded-[2rem] p-6 md:p-7 shadow-xl shrink-0 border border-white/10 isolate';
  if (estiloC === 'destacado') {
    cardPrecioClass = 'relative overflow-hidden bg-gradient-to-br from-slate-900 via-purple-950 to-slate-950 rounded-[2.5rem] p-7 shadow-2xl shrink-0 border border-purple-500/30 isolate ring-1 ring-purple-500/20';
  }
...
<template>
  ...
  <GuestLayout>
    ...
    >
     ...
     <RevealSection>
        <SectionPlantilla class="snap-start min-h-dvh lg:h-dvh w-full flex items-center justify-center bg-slate-50 relative py-20 lg:py-16">
          
          <div class="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-12 w-full" :class="layoutConfig.wrapper">
            
            <div :class="layoutConfig.temario">
              <div class="flex flex-col bg-white border border-slate-200 rounded-[2rem] md:rounded-[2.5rem] shadow-xl overflow-hidden h-full">
                
                <div class="px-6 py-4 md:px-8 md:py-6 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between shrink-0">
                  <h3 class="text-xl md:text-2xl font-black text-slate-900 flex items-center gap-3">
                    <div class="w-9 h-9 md:w-10 md:h-10 rounded-xl flex items-center justify-center shadow-inner"
                         :style="{ background: evento?.area_formacion?.color_hex_principal + '15', color: evento?.area_formacion?.color_hex_principal }">
                      <FileText class="w-5 h-5" />
                    </div>
                    Contenido Temático
                  </h3>
                  <BtnSecundario v-if="evento?.url_folleto" label="Folleto" icon="download" target="_blank" :href="'/storage/' + evento.url_folleto" :activeColor="evento?.area_formacion?.color_hex_principal" />
                </div>

                <div class="flex-1 overflow-y-auto p-6 md:p-8 custom-scroll">
                <div v-if="evento?.contenido_tematico?.modulos" :class="layoutConfig.temarioGrid">
  
  <div v-for="(modulo, index) in evento.contenido_tematico.modulos" :key="index" 
       :class="[
         'relative flex flex-col h-full bg-white border border-slate-200 rounded-[2rem] shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden',
         evento.estilo_temario === 'tarjetas' ? 'p-6' : ''
       ]">
    
    <div class="flex items-start gap-4 mb-5 shrink-0 min-h-[60px]">
      <span class="text-4xl font-black shrink-0 mt-[-4px]" :style="{ color: evento?.area_formacion?.color_hex_principal }">
        {{ index + 1 < 10 ? '0'+(index+1) : index+1 }}
      </span>
      <h4 class="text-[17px] font-bold text-slate-900 leading-snug w-full text-justify">
        {{ modulo.tema }}
      </h4>
    </div>

    <ul class="flex flex-col gap-2 flex-1 w-full">
      <li v-for="(subtema, subIndex) in modulo.subtemas" :key="subIndex" 
          class="flex items-start text-slate-600 text-[13px] leading-relaxed group">
        <CheckCircle class="w-4 h-4 mr-3 shrink-0 mt-0.5 opacity-70" :style="{ color: evento?.area_formacion?.color_hex_principal }" />
        <span class="text-justify">{{ subtema }}</span>
      </li>
    </ul>

    <div class="w-full h-1.5 mt-6 rounded-full opacity-80" 
         :style="{ background: evento?.area_formacion?.color_hex_principal }"></div>
  </div>
            ...
            <div :class="layoutConfig.sidebar">
              ...
              <div class="bg-white border border-slate-200 rounded-[2rem] md:rounded-[2.5rem] p-5 md:p-6 shadow-lg flex-1 flex flex-col min-h-0 overflow-hidden">
                <div class="mb-4 shrink-0">
                   <h3 class="text-xs md:text-sm font-black text-slate-900 uppercase tracking-widest flex items-center gap-2">
                      <Users class="w-4 h-4" :style="{ color: evento?.area_formacion?.color_hex_principal }" />
                      Equipo Académico
                   </h3>
                </div>
             ...
             <div class="flex-1 overflow-y-auto custom-scroll pr-1 mt-1">
                  <div :class="layoutConfig.expertosGrid">
                    ...
                    <div v-for="speaker in evento.conferencistas" :key="speaker.id" :class="layoutConfig.expertoCard">
                      
                      <div class="relative shrink-0" :class="{ 'mb-3': evento.estilo_expertos === 'tarjetas' }">
                        <img 
                          :src="speaker.foto ? '/storage/' + speaker.foto : `https://ui-avatars.com/api/?name=${speaker.primer_nombre}`" 
                          class="object-cover border-2 border-white shadow-sm z-10 transition-all duration-300 group-hover:scale-105"
                          :class="[
                            evento.estilo_expertos === 'tarjetas' ? 'rounded-full' : 'rounded-2xl',
                            evento.conferencistas?.length === 1 ? 'w-20 h-20 md:w-28 md:h-28' : 'w-16 h-16 md:w-20 md:h-20'
                          ]" 
                        />
                      </div>

                      <div class="min-w-0 flex-1 flex flex-col justify-center w-full" :class="{'items-center text-center': evento.estilo_expertos === 'tarjetas'}">
                        <span class="font-black uppercase tracking-widest block mb-1" 
                              :class="evento.conferencistas?.length === 1 ? 'text-[10px] md:text-xs' : 'text-[9px] md:text-[10px]'"
                              :style="{ color: evento?.area_formacion?.color_hex_principal }">
                          Consultor experto
                        </span>
                        
                        <h4 class="font-black text-slate-900 leading-tight transition-all"
                            :class="[
                              evento.conferencistas?.length === 1 ? 'text-lg md:text-xl' : 'text-sm md:text-base',
                              evento.estilo_expertos === 'tarjetas' ? 'w-full' : 'truncate w-full'
                            ]">
                          {{ speaker.primer_nombre }} {{ speaker.primer_apellido }}
                        </h4>
                        
                        <p class="text-slate-500 italic font-medium mt-1.5 transition-all"
                           :class="[
                             evento.conferencistas?.length === 1 ? 'text-xs md:text-sm line-clamp-4' : 'text-[11px] md:text-xs line-clamp-2'
                           ]">
              ...
              <div :class="layoutConfig.cardPrecio">
                <div class="absolute -top-10 -right-10 w-48 h-48 opacity-20 blur-[60px] pointer-events-none rounded-full"
                     :style="{ background: evento?.area_formacion?.color_hex_principal || '#f97316' }"></div>

                <div class="relative z-10 flex flex-col gap-5 h-full justify-between">
                  <div class="flex items-center justify-between">
                    <div>
                      <h3 class="text-xl font-black text-white tracking-tight">Asegura tu cupo</h3>
                      <p class="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Inversión para esta fecha</p>
                    </div>
                    <div class="flex items-center gap-2 bg-white/5 px-3 py-1.5 rounded-full border border-white/10 shrink-0">
                      <MapPin class="w-3.5 h-3.5" :style="{ color: evento?.area_formacion?.color_hex_principal }" />
                      <span class="text-[10px] font-bold text-slate-300 uppercase">{{ evento?.modalidad }}</span>
                    </div>
                  </div>
                  ...
                  <div class="flex items-center justify-between py-4 border-y border-white/5">
                    <div class="flex flex-col">
                      <p class="text-white text-3xl font-black tracking-tighter">
                        {{ formatPrice(evento?.precio_jornada || evento?.precio_seminario || evento?.precio_diplomado || 0) }}
                      </p>
                      <p class="text-[9px] text-slate-500 font-bold uppercase">Jornada Completa • IVA Incl.</p>
                    </div>
                    <div v-if="evento?.precio_modulo > 0" class="text-right border-l border-white/10 pl-4">
                      <p class="text-slate-300 text-lg font-bold">{{ formatPrice(evento.precio_modulo) }}</p>
                      <p class="text-[9px] text-slate-500 font-bold uppercase">Por Módulo</p>
                    </div>
                  </div>

                  <div v-if="evento?.ubicacion" class="flex items-center gap-2 text-slate-300">
                    <Info class="w-4 h-4 shrink-0" :style="{ color: evento?.area_formacion?.color_hex_principal }" />
                    <p class="text-[11px] font-medium italic">{{ evento.ubicacion }}</p>
                  </div>

                  <BtnUniversal @click="showModal = true" label="Inscribirme ahora" icon="send" icon-position="right" size="lg" :activeColor="evento?.area_formacion?.color_hex_principal" />
                </div>
              </div>

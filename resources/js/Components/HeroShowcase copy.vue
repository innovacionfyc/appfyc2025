...
<template>
  <section class="relative isolate h-[89dvh] w-full  py-5 flex items-end overflow-hidden bg-black">


    <div class="absolute inset-0">
      <div v-for="(event, index) in events" :key="event.id"
        class="absolute inset-0 bg-cover bg-center transition-opacity duration-1000 ease-in-out" :class="{
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 lg:gap-8 items-center">
        ...
        <aside class="lg:col-span-6 text-center lg:text-left">
          <transition name="slide-up" mode="out-in">
            <div :key="contentKey">
              <div
                class="inline-flex items-center gap-2 rounded-full bg-white/10 backdrop-blur-sm px-4 py-2 text-xs font-semibold text-white shadow-lg ring-1 ring-white/20">
                <span>{{ current?.badge || 'Evento' }}</span>
                <span class="opacity-50">•</span>
                <span class="opacity-80">{{ current?.city }}</span>
              </div>

              <h1 class="mt-4 text-4xl sm:text-5xl md:text-6xl font-extrabold text-white"
              ...
              <div class="mt-4 flex items-center justify-center lg:justify-start gap-1" v-if="current?.rating">
                <svg v-for="i in 5" :key="i" class="h-5 w-5"
                  :class="i <= (current?.rating || 0) ? 'text-yellow-400' : 'text-white/30'" viewBox="0 0 20 20"
                  fill="currentColor">
                  <path
              ...
              <div class="mt-8 flex flex-wrap justify-center lg:justify-start gap-4">
                <Link :href="current?.cta_url || '#'"
                  class="group inline-flex items-center gap-2 rounded-xl bg-primary-vinotinto px-6 py-3.5 text-base font-bold text-white shadow-xl transition-all duration-300 hover:bg-secondary-vinotinto2 hover:shadow-2xl hover:scale-105">
                  {{ current?.cta_text || 'Inscribirme' }}
                  <span class="transition-transform duration-300 group-hover:translate-x-1">→</span>
                </Link>
                <Link href="/oferta"
                  class="group inline-flex items-center rounded-xl bg-white/90 px-6 py-3.5 text-base font-semibold text-primary-vinotinto shadow-lg backdrop-blur transition-all duration-300 hover:bg-white hover:shadow-xl hover:scale-105">
                  Ver todos
                </Link>
              </div>
            </div>
            ...
            <div ref="rowRef"
              ...
              <div class="inline-flex gap-6 pr-24 pl-8 pt-8">
                <button v-for="(ev, i) in props.events" :key="ev.id || i" @click="setActive(i)"
                  ...
                  <div v-if="i === active && autoplay" class="absolute bottom-0 left-0 h-1 bg-white/80"
                    :key="progressKey" :style="{ animation: `progress ${intervalMs}ms linear` }"></div>
                </button>
              </div>
            </div>
          </div>
          <div class="flex justify-center gap-3 mt-4">
            <button @click="scrollBy(-480)" aria-label="Anterior"
              class="h-12 w-12 flex items-center justify-center rounded-full bg-white/20 backdrop-blur text-white ring-1 ring-white/30 transition hover:bg-white/30">
              <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
              </svg>
            </button>
            <button @click="scrollBy(480)" aria-label="Siguiente"
              class="h-12 w-12 flex items-center justify-center rounded-full bg-white/20 backdrop-blur text-white ring-1 ring-white/30 transition hover:bg-white/30">
              <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
.ken-burns {
  animation: kenburns 15s ease-out infinite alternate both;
}

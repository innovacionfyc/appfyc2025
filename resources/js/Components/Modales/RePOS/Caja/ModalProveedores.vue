...
const emit = defineEmits(["close", "edit", "delete", "proveedorActualizado"]);

const activeTab = ref(0);
const tabs = [{ label: "Directorio" }, { label: "Nuevo Registro" }];
...
function submit() {
  form.post(
    route("repos.proveedores.store", { aplicacion: props.aplicacion, rol: props.rol }),
    {
      preserveScroll: true,
      onSuccess: () => {
        activeTab.value = 0;
...
function closeModal(force = false) {
  if (form.isDirty && !force) {
    if (confirm("¿Descartar el nuevo proveedor?")) {
      emit("close");
      resetState();
    }
  } else {
...
function deleteProveedor(prov) {
  if (confirm(`¿Eliminar a "${prov.nombre}"?`)) {
    router.delete(
      route("repos.proveedores.destroy", {
...
<template>
  <BaseModalSteps
    ...
    :currentStep="1"
    :totalSteps="1"
    :title="activeTab === 0 ? 'Directorio de Aliados' : 'Registro de Aliado'"
    :description="
      activeTab === 0
        ? 'Administra tu red de abastecimiento y contactos.'
        : 'Completa los datos fiscales y de contacto.'
    "
    :isSubmitting="form.processing"
    :finalButtonText="activeTab === 0 ? 'Finalizar' : 'Guardar Proveedor'"
    :tabs="tabs"
    :activeTab="activeTab"

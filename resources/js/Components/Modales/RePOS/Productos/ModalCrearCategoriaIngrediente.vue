...
const props = defineProps({
  ...
  categoriasIngredientes: { type: Object, required: true },
});
const emit = defineEmits(["close", "edit"]);

// --- STATE ---
const activeTab = ref(0);
const tabs = [{ label: "Inventario de Suministros" }, { label: "Diseñar Categoría" }];
...
// --- ACCIONES ---
function submit() {
  form.post(route("repos.categoriasIngredientes.create"), {
    preserveScroll: true,
    onSuccess: () => {
      closeModal(true);
...
const confirmarEliminar = (cat) => {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      router.delete(route("repos.categoriasIngredientes.delete", cat.id), {
        preserveScroll: true,
        onSuccess: () => {
          Swal.fire({
            title: "¡Eliminado!",
...
<template>
  <BaseModalSteps
    ...
    :currentStep="1"
    :totalSteps="1"
    title="Categorización de Insumos"
    description="Organiza tus materias primas para un control de inventario exacto."
    :isSubmitting="form.processing"
    :finalButtonText="activeTab === 0 ? 'Cerrar' : 'Publicar Sección'"
    :tabs="tabs"
    :activeTab="activeTab"
    @update:activeTab="(val) => (activeTab = val)"
    @close="closeModal"
    @submit="isFormMode ? submit() : closeModal(true)"
    ...
    <div class="py-2">
      <Transition name="fade-slide" mode="out-in">
        <div
          v-if="activeTab === 0"
          key="catalog"
          class="flex flex-col h-full dark:bg-[#07090E]"
        >
          <div class="flex items-center justify-between mb-6 px-1">
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                  {{ categoriasIngredientes.total }} registros totales
                </p>
              </div>
            </div>
            ...
            <button
              @click="activeTab = 1"
              class="text-[10px] font-black text-primary uppercase border-b-2 border-primary/20 hover:border-primary transition-all pb-0.5"
            >
              + Añadir Nueva
            </button>
          </div>
          ...
          <div
            v-if="categoriasIngredientes.data?.length === 0"
          ...
          >
            <span
              class="material-symbols-rounded text-4xl text-gray-200 dark:text-gray-800 mb-2"
              >folder_off</span
            >
            <p class="text-xs font-bold text-gray-400 uppercase tracking-tighter">
              No hay categorías diseñadas
            </p>
          </div>
          ...
          >
            <div
              v-for="cat in categoriasIngredientes.data"
              :key="cat.id"
            ...
            >
              ...
              <div class="flex items-center gap-4 pl-2">
                ...
                <div class="flex flex-col min-w-0">
                  <h4
                    class="font-bold text-gray-900 dark:text-white text-sm tracking-tight truncate max-w-[150px]"
                  >
                    {{ cat.nombre }}
                  </h4>
                  <span
                    class="text-[9px] font-mono text-gray-400 font-bold uppercase truncate"
                  >
                    /{{ cat.slug }}
                  </span>
                </div>
          ...
          <div
            v-if="categoriasIngredientes.total > categoriasIngredientes.per_page"
          ...
          >
            <Pagination
              :links="categoriasIngredientes.links"
              :from="categoriasIngredientes.from"
              :to="categoriasIngredientes.to"
              :total="categoriasIngredientes.total"

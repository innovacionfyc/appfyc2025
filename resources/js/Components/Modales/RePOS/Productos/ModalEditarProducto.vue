...
// --- LÓGICA MAESTRA DE INGREDIENTES (BODREGA & STOCK) ---

/**
 * handleIngredientChange: La "fórmula secreta" corregida y optimizada.
 */
function handleIngredientChange(index) {
  const item = form.ingredientes[index];
  if (!item || !item.ingrediente_id) return;

  const master = getMasterData(item.ingrediente_id);
  if (!master) return;
  ...
  if (unitBodega) {
    // 1. Normalización a Unidad Base (ML / G)
    ...
    if (unitReceta) {
      const fBodega = Number(unitBodega.factor_conversion);
      const fReceta = Number(unitReceta.factor_conversion);
      const stockBodega = Number(master.stock_actual);
      const cantDigitada = Number(item.cantidad) || 0;

      // Cálculo matemático de consumo
      const consumoEnBodega = (cantDigitada * fReceta) / fBodega;

      // Proyección de Stock
      if (consumoEnBodega > stockBodega) {
        item.cantidad = (stockBodega * fBodega) / fReceta;
        item.stock_restante_texto = "0.000";
      } else {
        item.stock_restante_texto = (stockBodega - consumoEnBodega).toFixed(3);
      }
      item.unidad_bodega_abrev = unitBodega.abreviatura;
    }
  }
}

// --- COMPUTED PROPERTIES (REACTIVIDAD PURA) ---

const esSIMPLE = computed(() => {
  const tipo = props.TipoProductoDisponibles.find((t) => t.id == form.tipo);
  return tipo?.text.toUpperCase().includes("SIMPLE") || form.tipo === "SIMPLE";
});

const isRecipe = computed(() => {
  const tipo = props.TipoProductoDisponibles.find((t) => t.id == form.tipo);
  return (
    tipo?.text.toUpperCase().includes("RECETA") ||
    tipo?.text.toUpperCase().includes("COMPUESTO") ||
    form.tipo === "COMPUESTO"
  );
});
...
const maxProduction = computed(() => {
  ...
  const capacidades = form.ingredientes.map((item) => {
    const master = getMasterData(item.ingrediente_id);
    if (!master || !item.cantidad || Number(item.cantidad) <= 0) return 999999;
    return Math.floor(Number(master.stock_actual) / (Number(item.cantidad) / 1000));
  });
...
const valorAhorro = computed(() =>
  Math.round(
    (Number(form.precio) || 0) * ((Number(form.porcentaje_descuento) || 0) / 100)
  )
);
const precioFinal = computed(() => (Number(form.precio) || 0) - valorAhorro.value);
const utilidad = computed(() => precioFinal.value - (Number(form.costo_base) || 0));
...
const getMasterData = (id) =>
  id ? props.ingredientesDisponibles.find((i) => Number(i.id) === Number(id)) : null;

const getUnitAbrev = (id) =>
  props.unidadesMedidaDispobibles.find((u) => Number(u.id) === Number(id))?.abreviatura ||
  "ud";

// --- WATCHERS (EL MOTOR DE DATOS) ---

/**
 * Watcher principal: Detecta cuando abres un producto para editar.
 */
watch(
  () => props.producto,
  (newProd) => {
    if (newProd && props.isOpen) {
      isFillingForm.value = true;

      // Mapeo de campos básicos con sanitización
      form.categoria_id = newProd.categoria_id;
      form.tipo = newProd.tipo;
      form.nombre = newProd.nombre || "";
      form.plu_item = newProd.plu_item || "";
      form.descripcion = newProd.descripcion || "";
      form.maneja_inventario = !!newProd.maneja_inventario;
      form.area_produccion_id = newProd.areas_produccion?.[0]?.id || null;
      form.precio = parseFloat(newProd.precio) || 0;
      form.stock_actual = parseFloat(newProd.stock_actual) || 0;
      form.stock_minimo = String(newProd.stock_minimo ?? "0");
      form.porcentaje_impuesto = parseFloat(newProd.porcentaje_impuesto) || 0;
      form.porcentaje_descuento = parseFloat(newProd.porcentaje_descuento) || 0;
      form.precio_incluye_impuesto = !!newProd.precio_incluye_impuesto;
      form.costo_base = parseFloat(newProd.costo_base) || 0;
      form.imagen_url = newProd.imagen_url || null;
      form.quitar_imagen = false;

      // Mapeo de ingredientes (Receta)
      form.ingredientes = newProd.receta
        ? newProd.receta.map((ing) => ({
            ingrediente_id: ing.id,
            cantidad: String(ing.pivot.cantidad ?? ""),
            unidad_medida_id: ing.pivot.unidad_medida_id,
            stock_restante_texto: "---",
            unidad_bodega_abrev: "",
          }))
        : [];

      // 🔥 FORZAR CÁLCULO DE STOCK DESPUÉS DE CARGAR
      nextTick(() => {
        form.ingredientes.forEach((_, idx) => handleIngredientChange(idx));
...
// Sincronización automática de costo y producción
watch(
  [costoRecetaCalculado, maxProduction, isRecipe],
  ([newCosto, newStock, recipeMode]) => {
    if (recipeMode && !isFillingForm.value) {
...
// Watcher de cambio de tipo
watch(
  () => form.tipo,
  (newType) => {
    if (isFillingForm.value) return;
    if (newType === "COMPUESTO" && form.ingredientes.length === 0) {
      agregarIngrediente();
...
function quitarImagen() {
  ...
  const input = document.getElementById("imagen_producto_editar");
...
const imagePreviewUrl = computed(() => {
  ...
  return form.imagen_url ? getFotoProductoUrlCompleta(form.imagen_url) : null;
});
  ...
  form
    .transform((data) => {
      ...
      if (esSIMPLE.value) {
        ...
        payload.stock_actual = payload.stock_actual ? Number(payload.stock_actual) : 0;
      } else {
        delete payload.stock_actual;
        delete payload.maneja_inventario;
      }

      return payload;
    })
    .post(route("repos.productos.update", props.producto.id), {
      preserveScroll: true,
      onSuccess: () => {
        closeModal(true);
        form.reset();
      },
      onError: (e) => console.error("Error update:", e),
    });
}
...
<template>
  <BaseModalSteps
    ...
    title="Estudio de Edición de Producto"
    @close="closeModal"
    @next="handleNext"
    @prev="currentStep--"
    @submit="submit"
            ...
            <Transition name="fade-slide" mode="out-in">
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          ...
                          <div
                            class="h-2 w-full bg-white/5 rounded-full overflow-hidden p-[2px] border border-white/5"
                          >
                            <div
                              class="h-full rounded-full transition-all duration-1000 ease-out shadow-[0_0_8px_rgba(var(--color-primary-rgb),0.5)]"
                              :class="
                                parseFloat(ing.stock_restante_texto) < 0
                                  ? 'bg-red-500 shadow-red-500/50'
                                  : 'bg-primary'
                              "
                              :style="{ width: `${calculatePercentage(ing)}%` }"
                            ></div>
                          </div>
                        </div>
                      </div>
              ...
              >
                ...
                >
                  ...
                  <input type="checkbox" v-model="form.maneja_inventario" label="Activar" />
                </header>
